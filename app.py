...
# =========================
st.markdown("---")
st.caption("SpecSense AI V1.0 | Qualité 4.0 | Inspiré IATF 16949")    border: 1px solid rgba(0,212,255,0.35);
    border-radius: 20px;
    padding: 20px;
}
div[data-testid="stMetricValue"] {
...
def ask_hf_ai(question):
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="Qwen/Qwen2.5-7B-Instruct",
            messages=[
                {"role": "system", "content": "Tu es un expert qualité automobile. Réponds en français simple avec des actions concrètes."},
...
"""
    ...
    with tab_msa1:
        if len(msa_data) > 0:
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=list(range(1, len(msa_data) + 1)), y=msa_data["Measurement"], mode="lines+markers"))
            fig.add_hline(y=mean_msa, line_dash="dash", annotation_text="Moyenne")
            fig.add_hline(y=ref, line_dash="dot", annotation_text="Référence")
...
"""
    ...
    with tab_grr:
        if len(msa_data) > 0:
            ...
            fig = px.box(df_grr, x="Operator", y="Measurement", color="Operator", template="plotly_dark")
            st.plotly_chart(fig, use_container_width=True)
...
"""
    ...
    with tab_linearity:
        if len(msa_data) > 0:
            ...
            fig = px.scatter(df_lin, x="Reference", y="Bias", color="Operator", trendline="ols", template="plotly_dark")
            st.plotly_chart(fig, use_container_width=True)
...
"""
    ...
    with tab_stability:
        if len(msa_data) > 0:
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=df_stab["Date_Time"], y=df_stab["Measurement"], mode="lines+markers"))
...
"""
    ...
    with tab_control:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=spc_work["Point"], y=spc_work["Measurement"], mode="lines+markers"))
        fig.add_hline(y=mean_spc, line_dash="dash", annotation_text="CL")
        fig.add_hline(y=ucl, line_dash="dash", annotation_text="UCL")
        fig.add_hline(y=lcl, line_dash="dash", annotation_text="LCL")
        fig.add_hline(y=usl, line_dash="dot", annotation_text="USL")
