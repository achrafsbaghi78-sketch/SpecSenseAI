...
st.set_page_config(page_title="SpecSense AI", page_icon="🎯", layout="wide", initial_sidebar_state="expanded")

st.markdown("""
<style>
.stApp{background:linear-gradient(135deg,#020617 0%,#07111f 45%,#0b1220 100%);color:white;}
.block-container{padding-top:2rem;padding-left:2rem;padding-right:2rem;}
section[data-testid="stSidebar"]{background:linear-gradient(180deg,#020617 0%,#07111f 100%) !important;border-right:1px solid rgba(255,255,255,.08);}
div[data-testid="stMetric"]{background:radial-gradient(circle at top left,rgba(0,212,255,.18),rgba(15,23,42,.92));border:1px solid rgba(0,212,255,.45);border-radius:22px;padding:20px;box-shadow:0 18px 40px rgba(0,0,0,.25);}
div[data-testid="stMetricValue"]{color:white !important;font-size:34px !important;font-weight:900 !important;}
div[role="radiogroup"] label{background:rgba(15,23,42,.65);border:1px solid rgba(148,163,184,.14);border-radius:14px;padding:10px 12px;margin-bottom:8px;}
</style>
...
def ask_hf_ai(question):
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "Tu es un expert qualité automobile. Réponds en français simple, professionnel et avec des actions concrètes."},
...
def load_data():
    ...
    required_cols = ["Date_Time", "Part_ID", "Operator", "Trial", "Measurement", "USL", "LSL", "Machine", "Defect_Type", "Severity", "Occurrence", "Detection"]
    ...
    invalid = df[df[numeric_cols].isna().any(axis=1)]
    if len(invalid) > 0:
...
msa_data = df[df["Part_ID"].astype(str).str.contains("MSA", case=False, na=False)]
spc_data = df[df["Part_ID"].astype(str).str.contains("SPC", case=False, na=False)]
if len(spc_data) == 0:
    spc_data = df.copy()
...
with st.sidebar:
    ...
    page = st.radio("", ["🏠 Tableau de bord", "📏 MSA", "📉 SPC", "🎯 Capabilité", "📊 Pareto", "⚠️ AMDEC", "🤖 IA"], label_visibility="collapsed")
    st.markdown("---")
...
with h2:
    ...
    <p style="margin:6px 0 0 0; color:#94a3b8; font-size:18px;">Plateforme intelligente de qualité industrielle</p>
    """, unsafe_allow_html=True)
st.markdown("---")
...
k4.metric("Cpk", f"{cpk:.2f}", "Non capable ⚠️" if cpk < 1.33 else "Capable ✅")
if cpk < 1:
...
if page_clean == "Tableau de bord":
    ...
    with col_a:
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=list(range(1, len(df) + 1)), y=df["Measurement"], mode="lines+markers", name="Mesures"))
        ...
        st.plotly_chart(fig, use_container_width=True, key="dashboard_evolution")
    with col_b:
        ...
        st.plotly_chart(fig, use_container_width=True, key="dashboard_distribution")
    context_dashboard = f"""
Moyenne = {mean_val:.4f}
...
"""
    ...
    tab_msa1, tab_grr, tab_bias, tab_linearity, tab_stability, tab_attribute = st.tabs(["MSA Type 1", "Gage R&R", "Bias", "Linearity", "Stability", "Attribute MSA"])
    with tab_msa1:
        ...
        if len(msa_data) > 0:
            ...
            st.plotly_chart(fig, use_container_width=True, key="msa_type1_chart")
            context_msa = f"""
Référence = {ref:.4f}
...
"""
    ...
    with tab_grr:
        ...
        if len(msa_data) > 0:
            ...
            st.plotly_chart(fig, use_container_width=True, key="msa_grr_box")
            context_grr = f"""
...
"""
    ...
    with tab_linearity:
        ...
        if len(msa_data) > 0:
            ...
            st.plotly_chart(fig, use_container_width=True, key="msa_linearity_scatter")
...
"""
    ...
    with tab_stability:
        ...
        if len(msa_data) > 0:
            ...
            st.plotly_chart(fig, use_container_width=True, key="msa_stability_chart")
...
"""
    ...
    with tab_attribute:
        ...
        st.plotly_chart(fig, use_container_width=True, key="msa_attribute_pie")
        context_attribute = f"""
OK = {ok_count}
NOK = {nok_count}
%OK = {agreement:.2f}
...
"""
    ...
    tab_control, tab_rules, tab_distribution, tab_capability, tab_machine, tab_ai = st.tabs(["Carte de contrôle", "Règles SPC", "Distribution", "Capabilité", "Machine / Opérateur", "Interprétation IA"])
    with tab_control:
        ...
        st.plotly_chart(fig, use_container_width=True, key="spc_control_chart")
    ...
    with tab_rules:
        ...
        if rule1 > 0:
            r1.error(f"❌ {rule1} point(s) hors contrôle")
        else:
            r1.success("✅ Règle 1 OK")
        if rule2:
            r2.warning("⚠️ 7 points du même côté")
        else:
            r2.success("✅ Règle 2 OK")
        if trend_detected:
            r3.warning("⚠️ Tendance détectée")
        else:
    ...
    with tab_distribution:
        ...
        fig.add_vline(x=usl, line_dash="dash", annotation_text="USL")
        fig.add_vline(x=lsl, line_dash="dash", annotation_text="LSL")
        st.plotly_chart(fig, use_container_width=True, key="spc_distribution_hist")
    with tab_capability:
    ...
    with tab_machine:
        ...
        with col_m:
            if "Machine" in spc_work.columns:
                ...
                st.plotly_chart(fig, use_container_width=True, key="spc_machine_box")
            else:
        ...
        with col_o:
            if "Operator" in spc_work.columns:
                ...
                st.plotly_chart(fig, use_container_width=True, key="spc_operator_box")
            else:
...
"""
    ...
    tab_kpi, tab_hist, tab_centering, tab_machine, tab_ai = st.tabs(["Indices Cp / Cpk", "Histogramme", "Centrage", "Machine / Opérateur", "Interprétation IA"])
    with tab_kpi:
        c1, c2, c3, c4 = st.columns(4)
    ...
    with tab_machine:
        ...
        with col_m:
            ...
            st.plotly_chart(fig, use_container_width=True, key="cap_machine_box")
        with col_o:
            ...
            st.plotly_chart(fig, use_container_width=True, key="cap_operator_box")
    with tab_ai:
        target = (usl + lsl) / 2
        decentrage = mean_val - target
        context_cap = f"""
...
"""
    ...
    if len(defects) > 0:
        ...
        fig.update_layout(title="Diagramme Pareto", template="plotly_dark", height=460, yaxis=dict(title="Nombre"), yaxis2=dict(title="Cumul %", overlaying="y", side="right", range=[0, 110]))
...
"""
    ...
    table_fmea = fmea[["Part_ID", "Defect_Type", "Severity", "Occurrence", "Detection", "RPN", "Statut", "Action"]].rename(columns={"Part_ID": "Référence pièce", "Defect_Type": "Type de défaut", "Severity": "Gravité", "Detection": "Détection"})
...
if st.button("Générer le rapport PDF"):
    ...
    with open(pdf_path, "rb") as f:
        st.download_button(label="📥 Télécharger le rapport PDF", data=f, file_name="rapport_qualite_specsense.pdf", mime="application/pdf")
